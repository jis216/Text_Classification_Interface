...
def read_files(tarfname):
    ...
    pos_words = "data/positive-words.txt" 
    neg_words = "data/negative-words.txt"
    ...
    sentiment.tfidf = TfidfVectorizer(max_df=1.0, min_df=2, ngram_range=(1,3),  max_features=15000, sublinear_tf=True, smooth_idf=False)
    sentiment.train_tfidf_vec = sentiment.tfidf.fit_transform(sentiment.train_data)
    sentiment.dev_tfidf_vec = sentiment.tfidf.transform(sentiment.dev_data)

    sentiment.trainX  = sentiment.train_tfidf_vec
    sentiment.devX  = sentiment.dev_tfidf_vec
...
def read_unlabeled(tarfname, sentiment):
    ...
    unlabeled.X = sentiment.tfidf.transform(unlabeled.data)
